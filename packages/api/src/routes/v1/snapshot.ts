...
router.get('/us', cacheMiddleware(300), async (c) => {
  try {
    ...
    const highestMovers = [...stockAssets].sort((a: any, b: any) => Math.abs(Number(b.change)) - Math.abs(Number(a.change)))
    ...
    const deu = macroRiskIndicators.find((r: any) => r.code === 'DEXUSEU')
    ...
    console.error('[Snapshot US]', err.message)
    c.status(500)
...
router.get('/cn', cacheMiddleware(300), async (c) => {
  try {
    const [
      ...
      // 4. 行业估值（按 industry 聚合 core_stock_indicator）
      query(
    ...
    console.error('[Snapshot CN]', err.message)
    c.status(500)
    return c.json({ success: false, error: err.message })
  }
})
