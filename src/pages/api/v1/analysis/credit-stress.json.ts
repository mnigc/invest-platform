...
interface CreditRateStressResponse {
  spreaHistory: { dates: string[]; series: { name: string; data: (number | null)[] }[] }
  combinedStress: {
  ...
  currentSpread: { baaAaa: number | null; hyOas: number | null; spreadZScore: number | null }
...
export const GET = withCache(async () => {
  try {
    const horizon = 365
    const [baaAaaRows, bbbOasRows, hofmhyRows, t10yRows, t2yRows] = await Promise.all([
      safeQuery(`SELECT period_date, value FROM indicator_data d JOIN indicators i ON i.id = d.indicator_id WHERE i.code = 'BAA10Y' AND i.region = 'US' AND d.value IS NOT NULL ORDER BY period_date ASC LIMIT $1`, [horizon]),
      safeQuery(`SELECT period_date, value FROM indicator_data d JOIN indicators i ON i.id = d.indicator_id WHERE i.code = 'BAMLC0A0CM' AND i.region = 'US' AND d.value IS NOT NULL ORDER BY period_date ASC LIMIT $1`, [horizon]),
    ...
    const baaAaaMap = new Map(baaAaaRows.map((r: Record<string, any>) => [toDateStr(r.period_date), Number(r.value)]))
    const bbbOasMap = new Map(bbbOasRows.map((r: Record<string, any>) => [toDateStr(r.period_date), Number(r.value)]))
    const hofmhyMap = new Map(hofmhyRows.map((r: Record<string, any>) => [toDateStr(r.period_date), Number(r.value)]))
    ...
    const allDates = [...new Set([
      ...baaAaaRows.map((r: Record<string, any>) => toDateStr(r.period_date)),
    ...
    for (const d of allDates) {
      const spread = baaAaaMap.get(d) ?? null
    ...
    const currentCreditSpread = baaAaaMap.get(latestDate) ?? null
    const currentOas = bbbOasMap.get(latestDate) ?? null
    const spreadZ = creditSpreads.length > 63 ? zScore(creditSpreads.slice(-63), currentCreditSpread ?? 0) : null

    const creditStress = currentCreditSpread != null ? Math.min(1, Math.max(0, (currentCreditSpread - 1.5) / 3)) : null
    ...
    if (currentCreditSpread != null) evidence.push(`BAA-AAA利差当前 ${currentCreditSpread.toFixed(2)}%`)
    if (corrVal != null && corrVal < -0.3) evidence.push(`信用利差与利率变化负相关 ${(corrVal ?? 0).toFixed(2)}`)
    if (spread10y2y != null && spread10y2y < 0) evidence.push(`收益率曲线倒挂 ${(spread10y2y ?? 0).toFixed(2)}%`)

    const data: CreditRateStressResponse = {
      spreaHistory: {
        dates: allDates,
        series: [
          { name: 'BAA-AAA信用利差', data: spreadSeries },
      ...
      currentSpread: { baaAaa: currentCreditSpread, hyOas: currentOas, spreadZScore: spreadZ != null ? +spreadZ.toFixed(2) : null },
