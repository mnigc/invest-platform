...
const TYPE_STYLE: Record<KnowledgeNode['type'], { color: string; bg: string; label: string }> = {
  ...
  asset: { color: '#f23645', bg: 'rgba(242,54,69,0.14)', label: '资产' },
  market: { color: '#8b5cf6', bg: 'rgba(139,92,246,0.14)', label: '市场含义' },
  regime: { color: '#10b981', bg: 'rgba(16,185,129,0.14)', label: '宏观状态' },
...
export function KnowledgeCanvas({ nodes, edges, height = 620, onSelect, selected }: TopicGraphProps) {
    ...
    return nodes.map((n, i) => {
      ...
      return {
        ...
        style: {
          ...
          padding: '10px 14px',
          borderRadius: 10,
          border: isSel ? `2px solid ${style.color}` : '1px solid rgba(128,128,140,0.25)',
  ...
  const reactEdges: Edge[] = useMemo(() => {
    return edges.map((e, i) => ({
      ...
      label: e.label,
      animated: false,
      style: { stroke: 'rgba(120,140,180,0.55)', strokeWidth: 1.5 },
      labelStyle: { fill: 'var(--text-muted)', fontSize: 10 },
      labelBgStyle: { fill: 'var(--bg-card)', fillOpacity: 0.8 },
      markerEnd: { type: MarkerType.ArrowClosed, color: 'rgba(120,140,180,0.7)', width: 14, height: 14 },
    }))
  }, [edges])
  ...
  return (
    <div style={{ width: '100%', height, borderRadius: 12, overflow: 'hidden', border: '1px solid var(--border-light)', background: 'var(--bg-card)' }}>
      <ReactFlow
        nodes={reactNodes}
        edges={reactEdges}
      ...
      >
        <Background color="rgba(128,140,180,0.12)" gap={22} size={1} />
