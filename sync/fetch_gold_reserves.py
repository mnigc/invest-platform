...
log = _setup_logger("fetch_gold_reserves")
...
def fetch_today_gold_price():
    ...
    try:
        r = with_retry(
            lambda: requests.get(GOLD_API_URL, timeout=HTTP_TIMEOUT),
...
def fetch_fred_series(series_id, start_date="2000-01-01"):
    ...
    try:
        r = with_retry(
            lambda: requests.get(FRED_URL, params=params, timeout=HTTP_TIMEOUT),
            timeout=HTTP_TIMEOUT, max_retry=3,
        )
        r.raise_for_status()
        data = r.json()
...
def fetch_akshare_cn_gold():
    ...
    try:
        df = with_retry(
            ak.macro_china_foreign_exchange_gold,
            timeout=60, max_retry=2,
        )
    except Exception as e:
...
def main():
    ...
    try:
        ...
        write_sync_log("gold_reserves", "success", sum(counts.values()), summary)
        log.info("=" * 60)
        log.info("完成: %s", summary)

    except Exception as e:
        log.error("同步失败: %s", e, exc_info=True)
        write_sync_log("gold_reserves", "failed", 0, str(e))
        raise
